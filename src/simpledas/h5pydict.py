            ...
            for key, val in tree.items():
                if isinstance(val,dict):
                    ...
                    try:
                        ...
                        if isinstance(val,np.ndarray) and val.size==1 and val.dtype.char=='U': #convert string array to str 
                            parentgroup.create_dataset(str(key),data=str(val))                            
